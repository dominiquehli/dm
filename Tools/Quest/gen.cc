...
//    tax->display(fp);
    ...
    if (tax != NULL) {
        if (cum_prob[0] * nitems > 25)
            fp << 0 << "  " << cum_prob[0] * nitems << " " << tax->first_child(0) << " "
                << tax->last_child(0) << endl;
...
CustSeq::CustSeq(Cid cust_id, LINT seq_len, LINT trans_len)
    : cid(cust_id), slen(seq_len), tlen(trans_len), nitems(0), ntrans(seq_len), maxsize(
        5 * seq_len)
{
