...
Transaction *mk_tran(StringSetIter &lits, LINT tlen, LINT NPATS, Taxonomy *tax = NULL); //MJZ: added NPATS
...
void gen_rules(TransPar &par)
{
    ...
    for (LINT i = 0; i < par.ntrans; i++) {
        ...
        if (trans->size() > 0)
            NTRANS++; //added by me: repeat if trans empty
        else
            i--;
        delete trans;
    }
...
//  pat_fp << endl << endl << "Sequences:" << endl;
    ...
    for (i = 0; i < par.ncust; i++) {
        ...
        if (cust->cid < par.mincustid)
            par.mincustid = cust->cid;
        if (cust->cid > par.maxcustid)
            par.maxcustid = cust->cid;
