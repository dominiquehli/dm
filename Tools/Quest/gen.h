...
public:
    ...
    TransPar(void)
        : ntrans(1000000), tlen(10), nitems(100000), ascii(FALSE), seed(INIT_SEED), mintid(
            INT_MAX), maxtid(INT_MIN)
    {
    }
...
public:
    ...
    SeqPar(void)
        : ncust(100000), slen(10), tlen(2.5), nitems(DEFNITEMS), rept(0), rept_var(0.1), ascii(
            FALSE), mincustid(INT_MAX), maxcustid(INT_MIN) //MJZaki
    {
        lits.npats = 25000;
        lseq.npats = 5000;
        lits.patlen = 1.25;
...
public:
    ...
    {
        return items[n];
    } // return nth item of the string
    FLOAT conf_lvl(void)
    {
        return conf;
    }

    void set_size(LINT newsize)
    {
        nitems = newsize;
    }
    void set_item(LINT n, Item itm)
    {
        items[n] = itm;
    }
    void set_conf_lvl(FLOAT newconf)
    {
        conf = newconf;
    }
...
public:
    StringSetIter(StringSet &str_set)
        : strset(&str_set), last_pat(0)
    {
    }
    ;
...
public:
    ...
    static void set_print_cid(BOOLEAN flg = TRUE)
    {
        print_cid = flg;
    }

    static void set_tid(LINT ntid)
    {
        tid = ntid;
    }
