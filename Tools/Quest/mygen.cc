...
void Generate_Item_Sets()
{
    ...
    for (i = 1; i < Num_Large_Itemset; i++) {
        ...
        if (corr_items > Tau[i].size)
            corr_items = Tau[i].size;
        if (corr_items > Tau[i - 1].size)
            corr_items = Tau[i - 1].size;
        for (k = 0; k < Number_Items; k++)
            bitvec[k] = 0;
        for (j = 0; j < corr_items; j++) {
            rand_temp = (int) (Tau[i - 1].size * random_gen());
            /*printf("rand_tmp %d\n", rand_temp);*/
            if (bitvec[Tau[i - 1].items[rand_temp]] == 1)
                j--;
            else {
    ...
    if (binary_output) {
        ...
        for (p = 0; p < nproc; p++) {
            if (!fname)
                ...
                if (nproc > 1)
                    sprintf(filen, "%s.data.P%d", fname, p);
                else
